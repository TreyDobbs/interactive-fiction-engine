...
public:
	...
	//Function to search item from list and print the name
	void printDescription(string name) const
	{
		nodeType<Type>* current; 
		current = first;    

		//Check whether item was found in the inventory or not
		bool check = false;

		while (current != NULL) 
		{
			if (current->info->getName()==name) {
				cout<<current->info->getDesc();
				check = true;
				break;
			}
			current = current->link;
		...
		if (first == NULL) {
			...
			cout << "No item by that name in your inventory.";
		}
	}
